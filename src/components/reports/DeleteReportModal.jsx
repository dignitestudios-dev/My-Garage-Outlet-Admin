...
const DeleteReportModal = ({ eventDetails, toggleModal, handleDelete }) => {
    const [deleteReason, setDeleteReason] = useState(''); // state to store the reason

    // Handle change in the delete reason input
    const handleReasonChange = (e) => {
        setDeleteReason(e.target.value);
    };

    // Handle delete action, passing the reason
    const handleConfirmDelete = () => {
        // Pass the reason to the handleDelete function
    ...
    return (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-gray-800 bg-opacity-50 border border-gray-700 backdrop-blur-md shadow-lg rounded-xl p-6 w-1/3">
                <h3 className="text-xl font-semibold mb-4">Are you sure you want to delete this content?</h3>
